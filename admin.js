...
async function load(){
 ...
 if(!admin){
   const {data:boot,error}=await supabase.rpc('bootstrap_statkick_owner');
   if(!error&&boot){admin={active:true,is_owner:true};}
 }
 ...
 const [{count:users},{count:pools},{count:entries},{data:pay},{data:poolRows},{data:admins}]=await Promise.all([
  ...
  supabase.rpc('list_statkick_admins')
 ]);
 ...
 if(admin.is_owner){
  document.querySelector('#admin-form').onsubmit=async e=>{e.preventDefault();const msg=document.querySelector('#admin-msg');msg.textContent='Adding administrator…';const {error}=await supabase.rpc('add_statkick_admin_by_email',{p_email:document.querySelector('#admin-email').value.trim()});msg.textContent=error?`Could not add administrator: ${error.message}`:'Administrator added successfully.';if(!error)await load();};
  document.querySelectorAll('.remove-admin').forEach(btn=>btn.onclick=async()=>{if(!confirm('Remove this administrator’s access?'))return;btn.disabled=true;const {error}=await supabase.rpc('remove_statkick_admin',{p_user_id:btn.dataset.id});if(error){alert(error.message);btn.disabled=false;}else await load();});
 } else {document.querySelector('#admin-form').style.display='none';}
